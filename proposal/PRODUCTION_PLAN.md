# Arboryx — Production Implementation Plan

> **Last updated:** 2026-04-26
> **Tracks:** (A) Backend & Admin hardening — Phases 1–5 below.
> **Tracks:** (B) Public Landing Site — added 2026-04-25 from RFP, see next section.
> **Team assumption:** 1-2 developers, pragmatic choices over enterprise ceremony

---

## Track B — Public Landing Site (added 2026-04-25)

The public-facing landing + grove visualization. Full RFP and design
decisions live in companion files in this directory:

- `rfp.txt` — original requirements
- `response_1.txt` — questions raised, ambiguities flagged, improvements suggested
- `response_2.txt` — locked plan with all 11 ambiguities resolved (THIS is the
  authoritative phase plan for Track B)

**Track B status (as of 2026-04-26):**

| Phase | Description | Status |
|---|---|---|
| 1 | API read-path hardening (read-only key tier, `?action=entry`, `tooltip` field) | ✅ shipped |
| 2 | Tooltip backfill util + admin override field | ✅ shipped (807/807 entries backfilled) |
| 2.5 | Upstream PR to arboryx.ai pipeline (permanent fix for tooltip) | ✅ shipped (2026-04-27) |
| 3 | Public landing scaffold (`/frontend/`, design tokens, layout, config + deploy.sh) | ✅ shipped |
| 4 | Grove visualization — real leaves from data, hover audio, keyboard nav | ✅ shipped |
| 5 | `new_growth.html` (read-only mirror of admin, deep-link params) | ✅ shipped (2026-04-26) |
| 6 | Deploy pipeline integration | ✅ folded into Phase 3 (`frontend/deploy.sh`) |
| 7 | Power-user polish (time slider, ticker search, sparklines) | deferred (v2) |
| 8 | Real auth (separate RFP) | deferred |

See `deploy_operating_manual.md` (repo root) for current deployment commands.

---

## Track A — Backend & Admin Hardening (original plan)

> Original scope dated 2026-04-09. Phases 1-5 below pre-date Track B and
> are still relevant for the API/admin pillar.

## Current State (Phase 0 — ✅ shipped)

Phase 0 delivered the baseline API hardening on top of the existing Cloud Function:
...
**What Phase 0 produced:** A single Cloud Function reading flat JSON from GCS, protected by a static API key, with basic caching and pagination. Track B Phase 1 then split the key into a write-tier (admin) and read-tier (public) for the public landing.

**Track A Phase 1 (in progress, 2026-04-26)** is hardening this baseline: keys move into Secret Manager, CORS tightens to an allowlist, in-function rate limiting + max-instances cap added, input regexes hardened, on-demand `rotate_key.sh`, scheduled rotator for the admin key.
...
- Update `cloud_function/deploy.sh` to grant the service account `roles/secretmanager.secretAccessor` on the specific secret.
...
**Why now:** With three functions (API, sync, backup) and growing config, manual `bash cloud_function/deploy.sh` is error-prone. Before adding frontend features (Phase 5), make the deploy pipeline trustworthy.
    ...
    steps:
      ...
      - uses: google-github-actions/auth@v2
        with:
          credentials_json: ${{ secrets.GCP_SA_KEY }}
      - uses: google-github-actions/setup-gcloud@v2
      - run: bash cloud_function/deploy.sh
```
