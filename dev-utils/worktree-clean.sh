...
#
# Usage:
#   dev-utils/worktree-clean.sh                  # clean ALL merged worktrees
#   dev-utils/worktree-clean.sh <name>           # clean one (dir under .claude/worktrees/)
...
_sweep_orphan_branches() {
  ...
  for ref in $(git for-each-ref --format='%(refname:short)' 'refs/heads/worktree-*' 2>/dev/null); do
  ...
  # remote-only worktree-* branches (no local ref)
  for ref in $(git ls-remote --heads origin 'worktree-*' 2>/dev/null | awk '{sub("refs/heads/","",$2); print $2}'); do
...
if [ -n "$NAME" ]; then
  ...
  # orphaned merged worktree-* branches.
  found=0
