...
CONFIG_FILE="$SCRIPT_DIR/ui_config.config"
...
if [[ -z "$API_URL" ]]; then
    err "API_URL is not set in ui_config.config"
    exit 1
fi
if [[ -z "$API_KEY" ]]; then
    err "API_KEY is not set in ui_config.config"
    exit 1
fi

# Check that source file has placeholders (not already injected)
if ! grep -q '__ALPHASNAP_API_URL__' "$UI_PATH" || ! grep -q '__ALPHASNAP_API_KEY__' "$UI_PATH"; then
...
echo ""
echo -e "${BOLD}============================================${RESET}"
echo -e "${BOLD}  AlphaSnap UI — Deploy${RESET}"
echo -e "${BOLD}============================================${RESET}"
echo ""
...
sed -e "s|__ALPHASNAP_API_URL__|${API_URL}|g" \
    -e "s|__ALPHASNAP_API_KEY__|${API_KEY}|g" \
    "$UI_PATH" > "$TMPFILE"
